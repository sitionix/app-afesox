...
These entries generate a separate frontend-oriented TypeScript contract package and publish it to GitHub npm packages.
...
apis/<service>/event/metadata.yml
...
Implement the generated handler interface as a Spring bean. The auto-configuration wires the
`${EventTag}${Version}Consumer` and starts a `${EventTag}${Version}ConsumerRunner` that polls and
dispatches records to your handler.

```java
@Component
public class EmailVerifyHandler implements EmailverifyV1ConsumerHandler {
    @Override
    public void consumeEmailVerify(EmailVerifyEventEnvelope event) {
        // handle event
    }
}
```

If you prefer manual control, inject `EmailVerifyV1Consumer` and call `pollOnce(...)` yourself.

Notes:
- Avro generates the event POJOs; Kafka producer/consumer wrappers are generated from `apis/<service>/event/asyncapi.yml`.
- Wrapper classes are generated by tag; `api.version` in `apis/<service>/event/metadata.yml` must match `asyncapi.info.version`.
- The wrappers accept a value serializer/deserializer, so you can plug in your Kafka Avro serialization strategy.
- Event wrapper generation requires `yq`, and schema staging uses `python3` on the build machine.
